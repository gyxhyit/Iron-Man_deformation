...
namespace Tbx {
    ...
    */
    struct Transfo {
        ...
        inline Transfo(const Mat3& x) {             //接受一个Mat3对象作为参数，然后将其转换为一个Transfo对象
        ...
        inline Transfo(const Mat3& x, const Vec3& v) {  // 不同之处在于它还接受了一个平移向量v
        ...
        inline Transfo(const Vec3& v) {                //其他位置保持单位矩阵的值
        ...
        inline Vec3 x() const { return Vec3(m[0], m[4], m[8]); }   //返回矩阵的x、y、z轴
        inline Vec3 y() const { return Vec3(m[1], m[5], m[9]); }
        inline Vec3 z() const { return Vec3(m[2], m[6], m[10]); }


        inline Mat3 get_mat3() const {   //返回一个3x3矩阵，表示Transfo对象的旋转部分。
        ...
        inline Vec3 get_translation() const { return Vec3(m[3], m[7], m[11]); }  //返回矩阵的平移部分或者原点位置。
        ...
        inline void set_x(const Vec3& x) { m[0] = x.x; m[4] = x.y; m[8] = x.z; }//分别设置矩阵的x、y、z轴。
        inline void set_y(const Vec3& y) { m[1] = y.x; m[5] = y.y; m[9] = y.z; }
        inline void set_z(const Vec3& z) { m[2] = z.x; m[6] = z.y; m[10] = z.z; }

        inline void set_translation(const Vec3& tr) { m[3] = tr.x; m[7] = tr.y; m[11] = tr.z; }//设置矩阵的平移部分
        inline void set_org(const Vec3& tr) { m[3] = tr.x; m[7] = tr.y; m[11] = tr.z; }//设置矩阵的原点位置


        inline void set_translation(const Transfo& tr) {   //根据另一个Transfo对象的平移部分设置当前对象的平移部分。
            const Vec3 trans = tr.get_translation();
            m[3] = trans.x; m[7] = trans.y; m[11] = trans.z;
        }


        inline void set_mat3(const Mat3& x) {    //根据给定的3x3矩阵设置当前对象的旋转部分
